...
- **Map pins are one visual family — and now one renderer.** Saved pins, AI-suggestion pins, and clusters all render as the same rounded **photo-tile** (40pt rounded square, photo or category-fallback image, little pointer to the coordinate). They differ only in what fills the tile (photo / category icon / count), the pointer-stem colour, and an optional corner badge: AI-suggestion pins get a **lime stem + sparkle badge**, events get an **amber clock badge**, clusters show a **count**. **All three funnel through one shared renderer — `MapPinTile.render(content:edge:edgeWidth:badge:)` in `ClusteredMapView.swift`** (the per-class `renderTile` / `renderCountTile` methods are now thin adapters that just supply content + colour). Change the pin look in `MapPinTile` and every pin updates together — do NOT re-fork the geometry back into a caller. The tile wears the classic frame: a 2.5pt edge ring (white for saved pins/clusters, lime for AI pins) around the photo plus a 0.5pt dark hairline. `edgeWidth` defaults to 2.5; pass 0 for an edge-to-edge variant. (An edge-removal pass briefly set this to 0, then reverted — the frame is the intended look.)
- **Image-bearing surfaces wear the defined thin edge.** Anything with an image (or image placeholder) — the card/tile AND the photo box inside it — gets the same modern, crisp outline: a ~1pt light-gray line (`Color.somedayEdge`, defined once in `TileStyle.swift`). Two modifiers apply it:
  - **`.photoTileEdge(cornerRadius:lineWidth:)`** — on the image/placeholder box. Clips to the rounded rect and strokes a 1pt `somedayEdge` border (no white inset frame — the line *is* the edge). Use it instead of clipping straight to a `RoundedRectangle`.
  - **`.somedayCardEdge(cornerRadius:lineWidth:)`** — on the outer card/tile surface, so the whole element and the photo share one outline language.

  Applied to `map_pin_tile` (PlaceCardSheet: 96pt hero + card), `introduction_tile` (ImportSummaryCardView: 160pt photo; the slide sits directly on the floating glass tile so it needs no separate card border), and `chatbot_tile` (ChatPlacePeekCard: hero + card). New image surfaces should follow the same pattern. NB: the *map-pin renderer* (`MapPinTile` in `ClusteredMapView.swift`) draws its own Core Graphics geometry (rounded tile + white/lime edge + pointer + hairline) — that's the pin geometry, separate from this SwiftUI edge treatment.
...
- **Brand mark — the hot-air balloon.** The balloon is the app's logo. There are four balloon assets in `Assets.xcassets`, each with a distinct job — don't mix them up:
  - **`airballoonNoBackground`** — the crisp, transparent cut-out. The canonical *clean* brand mark. Reach for this one on any new surface that needs a sharp logo over a colored/gradient background.
  - **`airballoonPixelated`** — a transparent, pixel-art version of the cut-out. Currently the mark crowning the auth `hero` (72pt, directly above the `Someday` wordmark in `AuthView.swift`) for a playful, retro feel. Same compositing rules as the clean cut-out (carries its own alpha).
  - **`balloon`** — the soft white silhouette rendered large + faded behind the auth form (centred, `offset(y: -120)`, opacity ~0.92). It's atmospheric background texture, not a foreground logo.
  - **`shareButtonAirballoon`** — bespoke art for the Share-Extension button; not a general-purpose mark.
  When placing the mark, prefer one of the transparent cut-outs (`airballoonNoBackground` for sharp, `airballoonPixelated` for retro) so it composites cleanly (each carries its own alpha — no `foregroundStyle` tint needed).
